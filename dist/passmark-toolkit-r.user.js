...
(function () {
  ...
  for (let liIndex = 0; liIndex < lis.length; liIndex++) {
    let li2 = lis[liIndex];
    let mark2 = $(".count", li2).text().replace(",", "");
    let price = $(".price-neww", li2).text().replace(",", "").replace("$", "").replace("*", "");
    let product = {
      id: Number(li2.id.substring(2)),
      link: $("a", li2).attr("href"),
      name: $(".prdname", li2).text(),
      mark: parseInt(mark2),
